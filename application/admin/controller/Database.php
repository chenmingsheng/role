...
class Database extends Common{
    ...
    public function initialize() {
        ...
        $this->db = new Baksql($conf['database']);
    }
    ...
    public function backup() {
        ...
        if ($type == 1) {
            ...
            if (!$res) $this->inlog('单表备份数据库',$tabName[0]."：备份失败");
            $this->inlog('单表备份数据库', $tabName[0] ."：备份成功");
        # 完全备份
        }else{
            $tables = $this->db->get_dbname();    // 全部表
            $res    = $this->db->backup($tables); // 开始备份
            if (!$res) $this->inlog('完全备份数据库', "备份失败");
            $this->inlog('完全备份数据库', "备份成功");
        }
    }
    ...
    public function download() {
        ...
        if ($type == 1) {
            ...
            $this->inlog('下载单表备份文件', $table);
        # 完全下载
        }else{
            $table = $this->db->get_completely();// 获取最新的完全备份
            if (!$table) $this->inlog('下载完全备份文件','获取失败，暂无最新备份文件');
            $this->db->backup($table , $table);  // 进行最新备份
            $tabName = $table.".sql";            // 获取表名
            $this->db->downloadFile($tabName);   // 下载文件
            $this->inlog('下载完全备份文件', '下载成功');
        }
    }
    ...
    public function restore(){
        ...
        if ($type == 1) {
            ...
            if ($size > 200) $this->inlog('备份还原', $table.'：暂不支持还原大于200KB的备份文件');
            $this->db->_import($table);
            $this->inlog('备份还原', '还原成功');
        }
    }
